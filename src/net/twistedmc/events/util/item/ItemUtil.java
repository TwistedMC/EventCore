...
public class ItemUtil {
    public static LinkedList<Map.Entry<Material, Byte>> matchItem(Player caller, String items, boolean inform) {
        ...
        if (inform && failList.length() > 0) {
            ...
            caller.sendMessage(c.red + "Invalid item! Did you mean: " + c.yellow + failList);
        }
        return matchList;
    }
    ...
    public static Map.Entry<Material, Byte> searchItem(Player caller, String args, boolean inform) {
        ...
        if (matchList.size() != 1) {
            if (!inform)
                return null;
            caller.sendMessage(c.red + "Invalid item! Did you mean: " + c.yellow + args);
            if (matchList.size() > 0) {
                String matchString = "";
                ...
                caller.sendMessage(c.red + "Invalid item! Did you mean: " + c.yellow + matchString);
            }
            return null;
        }
